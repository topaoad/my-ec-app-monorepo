...
type FaviritesItem = Product & MicroCMSContentId & MicroCMSDate;

const FaviritesBody: FC<FaviritesProps> = ({ products }) => {
  const [faviritesItems, setFaviritesItems] = useState<FaviritesItem[]>([]);
  ...
  return (
    <div className="container mx-auto px-4 max-w-7xl">
      ...
      {faviritesItems.length === 0 ? (
        <p className="text-center">カートは空です。</p>
      ) : (
        <div className="space-y-4">
          {faviritesItems.map(item => (
            <Card key={item.id}>
              <CardHeader>
          ...
          <div className="text-right">
            <p className="text-xl font-bold">
              合計: {faviritesItems.reduce((sum, item) => sum + item.price, 0).toLocaleString()}円
            </p>
